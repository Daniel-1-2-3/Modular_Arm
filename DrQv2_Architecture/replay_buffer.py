...
# MIT license (DrQv2-style episodic replay), adapted for dict observations (pov/tof).
...
class ReplayBufferStorage:
    """
    Stores episodes as .npz files with arrays:
      pov:    (T, C, H, W) float32
      tof:    (T, 1) float32
      action: (T, A) float32
      reward: (T, 1) float32
      discount:(T,1) float32
    """
    ...
    def add(self, time_step: ExtendedTimeStep):
        for spec in self._data_specs:
            ...
            assert spec.shape == value.shape and spec.dtype == value.dtype, (
                f"{spec.name}: expected {spec.shape}/{spec.dtype}, got {value.shape}/{value.dtype}"
            )
        ...
        if time_step.last():
            episode = {}
            for spec in self._data_specs:
                episode[spec.name] = np.array(self._current_episode[spec.name], dtype=spec.dtype)
            self._current_episode = defaultdict(list)
...
class ReplayBuffer(IterableDataset):
    def __init__(self, replay_dir: Path, max_size: int, num_workers: int,
                 nstep: int, discount: float, fetch_every: int, save_snapshot: bool):
        self._replay_dir = replay_dir
        self._size = 0
    ...
    def _store_episode(self, eps_fn: Path):
        try:
    ...
    def _sample(self):
        ...
        # +1 for the first dummy transition
        idx = np.random.randint(0, episode_len(episode) - self._nstep + 1) + 1

        obs = {
            "pov": episode["pov"][idx - 1],
            "tof": episode["tof"][idx - 1],
        }
        action = episode["action"][idx]
        next_obs = {
            "pov": episode["pov"][idx + self._nstep - 1],
            "tof": episode["tof"][idx + self._nstep - 1],
        }
